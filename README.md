...
- 15 core Python agents plus 116 extended Markdown playbooks are loaded on demand with
  caching, matching, and delegation via `AgentLoader` and `ExtendedAgentLoader`.
...
- Most command outputs describe planned work (plus generated evidence files) rather than
  editing source code automatically; human review is still required.
- Extended agent registry ships many Markdown playbooks, but only the 15 core agents have
  Python implementations that run code.
