                ...
                {move || {
                    ...
                    if execs.is_empty() {
                        ...
                        execs.into_iter().map(|exec| {
                            ...
                            view! {
                                <div class="exec-item" style="flex-wrap: wrap;">
                                    ...
                                    <div style="display: flex; gap: 6px;">
                                        {if is_running {
                                            ...
                                            Some(view! {
                                                <button class="btn btn-secondary" on:click=move |_| pf(ep.clone())>"Pause"</button>
                                            })
                                        } else { None }}
                                        {if is_paused {
                                            ...
                                            Some(view! {
                                                <button class="btn btn-primary" on:click=move |_| rf(er.clone())>"Resume"</button>
                                            })
                                        } else { None }}
                                        {if is_running || is_paused {
                                            ...
                                            Some(view! {
                                                <button class="btn btn-danger" on:click=move |_| sf(es.clone())>"Stop"</button>
                                            })
                                        } else { None }}
                                    </div>
                                </div>
