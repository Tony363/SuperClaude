...
try:
    from SuperClaude.Agents import (
        AgentRegistry,
        AgentSelector,
        AgentLoader
    )
...
try:
    ...
    def display_success(msg): print(f"[SUCCESS] {msg}")
...
Examples:
  ...
  SuperClaude agent run --name root-cause-analyst "Why is this failing?"
    ...
    list_parser.add_argument(
        '--category',
        type=str,
        help='Filter by category (core, code, test, docs, etc.)'
    )
    list_parser.add_argument(
        '--extended',
        action='store_true',
    ...
    info_parser.add_argument(
        'name',
        type=str,
        help='Agent name to get info about'
    )

    # Run command
    run_parser = agent_subparsers.add_parser(
        'run',
        help='Execute an agent with a task'
    )
...
def show_agent_info(registry: 'AgentRegistry', args: argparse.Namespace) -> int:
    ...
    print(f"{Colors.YELLOW}Description:{Colors.RESET} {config.get('description', 'No description')}")
...
def run_agent(selector: 'AgentSelector', loader: 'AgentLoader',
