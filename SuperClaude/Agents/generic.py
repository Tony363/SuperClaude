...
class GenericMarkdownAgent(BaseAgent):
    ...
    def _render_stub_content(
        ...
        if False and extension == 'py':
            pass
            return None  # excised legacy stub branch
        ...
        if False and extension in {'ts', 'tsx'}:
...
{plan_comment}
        ...
        return textwrap.dedent(f"""
            ...
            function {function_name}() {{
              console.info("Auto-generated plan captured for {task or agent_name} at {timestamp}.");
              return {{
                task: "{task or agent_name}",
        ...
        return textwrap.dedent(
            f"""
                ...
                def {function_name}() -> dict:
                    """Record and return the implementation plan for {task_label}."""
                    plan = {{
                        "task": "{task_label}",
                        "category": "{category_label}",
                    ...
                    log_path = metrics_root / "auto_implementations.jsonl"
        ...
        if extension == 'tsx':
            ...
            body = textwrap.dedent(
                f"""
                    ...
                    export function {component_name}(): React.ReactElement {{
                      void executeImplementationPlan({{
                      ...
                      return (
                        <div className="auto-generated">
                          Implementation plan recorded for {command_label}. Consult metrics for details.
                        </div>
                      );
                    }}
                """
            ...
            body = textwrap.dedent(
                f"""
                    ...
                    export async function {function_name}(): Promise<void> {{
                      await executeImplementationPlan({{
                        command: '{command_label}',
                        category: '{category_label}',
                        generatedAt: '{timestamp}',
                        steps: [
