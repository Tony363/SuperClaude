...
class RefactoringExpert(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if 'description' not in config:
            config['description'] = 'Improve code quality through systematic refactoring'
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result['actions_taken'].append(f"Identified {len(opportunities)} improvement opportunities")
    ...
    def validate(self, context: Dict[str, Any]) -> bool:
        ...
        refactoring_keywords = [
            'refactor', 'improve', 'clean', 'optimize code',
            'restructure', 'simplify', 'reduce complexity',
    ...
    def _detect_code_smells(
        ...
        for keyword, smell_type in generic_smells.items():
            if keyword in task_lower and not any(s['type'] == smell_type for s in smells):
                smells.append({
                    'type': smell_type,
    ...
    def _generate_recommendations(
        ...
        if smells:
            for smell in smells[:5]:
                severity_icon = {'high': '🔴', 'medium': '🟡', 'low': '🟢'}.get(smell['severity'], '⚪')
                lines.append(f"- {severity_icon} **{smell['type'].replace('_', ' ').title()}**: {smell['description']}")
        ...
        if plan:
            for i, item in enumerate(plan[:5], 1):
                ...
                lines.append(f"   - Priority: {['High', 'Medium', 'Low'][min(item['priority']//3, 2)]}")
        ...
        lines.append(f"- **Maintainability Improvement**: {impact['maintainability_improvement']}%")
        lines.append(f"- **Risk Level**: {impact['risk_level'].capitalize()}")
        lines.append(f"- **Estimated Effort**: {impact['estimated_effort_hours']} hours")

        # Best practices section
        lines.append("\n## Best Practices\n")
        lines.append("1. **Test Coverage**: Ensure comprehensive tests before refactoring")
