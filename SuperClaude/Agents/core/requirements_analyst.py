...
class RequirementsAnalyst(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if 'description' not in config:
            config['description'] = 'Transform ambiguous ideas into concrete specifications'
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result['actions_taken'].append(f"Identified {len(clarifications)} clarifications needed")
            ...
            result['actions_taken'].append(f"Defined {len(criteria)} acceptance criteria")
            ...
            report = self._generate_requirements_report(
                task, requirements, clarifications, stories,
                criteria, specification, recommendations
            )
    ...
    def validate(self, context: Dict[str, Any]) -> bool:
        ...
        requirements_keywords = [
            'requirement', 'spec', 'specification', 'user story',
            'acceptance criteria', 'feature', 'scope', 'define',
            'clarify', 'analyze requirements', 'prd', 'brd'
        ]
    ...
    def _elicit_requirements(self, task: str, description: str) -> List[Dict[str, Any]]:
        ...
        functional_keywords = ['create', 'read', 'update', 'delete', 'search',
                              'filter', 'sort', 'export', 'import', 'generate']
    ...
    def _identify_clarifications(
        ...
        ambiguous_terms = ['some', 'many', 'fast', 'slow', 'big', 'small',
                          'good', 'bad', 'appropriate', 'suitable']
    ...
    def _define_acceptance_criteria(
        ...
        for story in stories[:10]:  # Limit to 10 stories
            ...
            criteria.append({
                'id': f'AC-{len(criteria)+1:03d}',
                'story_id': story['id'],
                'criteria': f"System validates all inputs and provides clear error messages",
                'type': 'validation',
                'testable': True
            })
            ...
            if story.get('priority') in ['critical', 'high']:
                criteria.append({
                    'id': f'AC-{len(criteria)+1:03d}',
                    'story_id': story['id'],
                    'criteria': f"Response time must be under 2 seconds",
                    'type': 'performance',
                    'testable': True
                })
    ...
    def _generate_specification(
        self, requirements: List[Dict[str, Any]], stories: List[Dict[str, Any]],
        criteria: List[Dict[str, Any]], constraints: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        specification = {
            ...
            'summary': {
                ...
                'functional': len([r for r in requirements if r['type'] == 'functional']),
                'non_functional': len([r for r in requirements if r['type'] == 'non_functional']),
                'user_stories': len(stories),
                'acceptance_criteria': len(criteria)
            },
            'priorities': {
                'critical': len([r for r in requirements if r.get('priority') == 'critical']),
                'high': len([r for r in requirements if r.get('priority') == 'high']),
                'medium': len([r for r in requirements if r.get('priority') == 'medium']),
    ...
    def _generate_requirements_report(
        self, task: str, requirements: List[Dict[str, Any]],
        clarifications: List[Dict[str, Any]], stories: List[Dict[str, Any]],
        criteria: List[Dict[str, Any]], specification: Dict[str, Any],
