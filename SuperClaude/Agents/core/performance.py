...
class PerformanceEngineer(BaseAgent):
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result['actions_taken'].append(f"Identified {len(issues)} performance issues")
            ...
            result['actions_taken'].append(f"Generated {len(optimizations)} optimizations")
    ...
    def validate(self, context: Dict[str, Any]) -> bool:
        ...
        performance_keywords = [
            'performance', 'slow', 'optimize', 'speed',
            'bottleneck', 'latency', 'throughput', 'efficiency',
            'profile', 'benchmark', 'scaling', 'resource'
        ]
    ...
    def _initialize_strategies(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            'caching': {
                'applies_to': ['repeated_computation', 'expensive_operation', 'missing_cache'],
    ...
    def _detect_bottlenecks(
        ...
        db_issues = [i for i in issues if 'database' in i['type'] or 'query' in i['type']]
        if db_issues:
            bottlenecks.append({
                'type': 'database',
        ...
        # CPU bottlenecks
        cpu_issues = [i for i in issues if 'cpu' in i['type'] or 'algorithm' in i.get('description', '').lower()]
        if cpu_issues:
            bottlenecks.append({
                'type': 'cpu',
                'description': 'CPU-intensive operations bottleneck',
                'impact': 'high' if any(i['severity'] == 'high' for i in cpu_issues) else 'medium',
                'issues': cpu_issues
            })
        ...
        # I/O bottlenecks
        io_issues = [i for i in issues if 'io' in i['type'] or 'synchronous' in i['type']]
        if io_issues:
            bottlenecks.append({
                'type': 'io',
        ...
        if any('nested_loops' in i['type'] for i in issues):
            bottlenecks.append({
                ...
                'impact': 'high',
                'issues': [i for i in issues if 'loop' in i['type'] or 'algorithm' in i['type']]
            })
    ...
    def _calculate_metrics(
        ...
        total_impact = sum(impact_scores.get(b.get('impact', 'medium'), 2) for b in bottlenecks)
        ...
        return {
            ...
            'priority_optimizations': sum(1 for o in optimizations if o['priority'] == 1)
        }

    def _generate_performance_report(
        self, task: str, issues: List[Dict[str, Any]],
        bottlenecks: List[Dict[str, Any]], optimizations: List[Dict[str, Any]],
        ...
        if issues:
            for issue in issues[:5]:
                ...
                lines.append(f"- {severity_icon} **{issue['type'].replace('_', ' ').title()}**: {issue['description']}")
            if len(issues) > 5:
        ...
        lines.append(f"- High-priority optimizations: {metrics['priority_optimizations']}")
