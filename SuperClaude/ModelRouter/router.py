...
class ModelRouter:
    ...
    def route(
        ...
        # Handle forced model
        if force_model:
            if force_model in self.MODEL_CAPABILITIES and self._is_available(
                force_model
            ):
                ...
                logger.warning(
                    f"Forced model {force_model} not available, routing normally"
                )
        ...
        preferred_models = self.TASK_PREFERENCES.get(
            task_type, self.TASK_PREFERENCES["standard"]
        )
        ...
        primary = self._select_primary_model(
            preferred_models, excluded_models, context_size
        )
        ...
        return self._create_decision(
            primary, fallback_chain, reason, context_size, think_level
        )
    ...
    def _route_long_context(
        ...
        for model_name, window_size in long_context_models:
            if (
                model_name not in excluded_models
                and window_size >= context_size
                and self._is_available(model_name)
            ):
    ...
    def _is_available(self, model_name: str) -> bool:
        ...
        # Update cache
        self.availability_cache[model_name] = (
            capability.availability > 0.5,
            datetime.now(),
        )
    ...
    def _get_routing_reason(
        self, task_type: str, think_level: int, context_size: int
    ) -> str:
    ...
    def record_usage(
        self, model: str, tokens_used: int, success: bool, task_type: str = "standard"
    ) -> None:
        """
        ...
        if not success and model in self.MODEL_CAPABILITIES:
            ...
            self.MODEL_CAPABILITIES[model].availability = min(
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        for model in stats["success_rate"]:
            ...
            stats["success_rate"][model] = (
                sum(success_list) / len(success_list) if success_list else 0
            )

            tokens_list = stats["average_tokens"][model]
            stats["average_tokens"][model] = (
                sum(tokens_list) / len(tokens_list) if tokens_list else 0
            )
