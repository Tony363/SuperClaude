...
class TestAskSkillSchema:
    ...
    def test_frontmatter_name_matches_directory(self, ask_skill_path: Path) -> None:
        ...
        assert frontmatter.get("name") == dir_name, (
            f"Frontmatter name '{frontmatter.get('name')}' should match directory '{dir_name}'"
        )
    ...
    def test_instructions_section_not_empty(self, ask_skill_path: Path) -> None:
        ...
        sections = parse_skill_sections(ask_skill_path)
        assert sections.get("Instructions", "").strip(), "Instructions section should not be empty"
...
class TestAskMultiSkillSchema:
    ...
    def test_frontmatter_name_matches_directory(self, ask_multi_skill_path: Path) -> None:
        """Frontmatter name should match directory name."""
        frontmatter = parse_skill_frontmatter(ask_multi_skill_path)
    ...
    def test_description_mentions_multiselect(self, ask_multi_skill_path: Path) -> None:
        """Description should mention multi-select capability."""
        frontmatter = parse_skill_frontmatter(ask_multi_skill_path)
        description = frontmatter.get("description", "").lower()
        assert "multi" in description, "Description should mention multi-select capability"
...
class TestAskSkillBehavior:
    ...
    def test_instructions_mention_options_constraint(self, ask_skill_path: Path) -> None:
        ...
        assert "2" in combined or "two" in combined.lower(), "Should mention minimum of 2 options"
        assert "4" in combined or "four" in combined.lower(), "Should mention maximum of 4 options"

    def test_instructions_mention_multiselect_false(self, ask_skill_path: Path) -> None:
...
class TestAskMultiSkillBehavior:
    ...
    def test_instructions_mention_multiselect_true(self, ask_multi_skill_path: Path) -> None:
